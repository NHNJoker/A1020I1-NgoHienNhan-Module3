...
<div class="container-fluid">
    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <form action="/customer?actionUser=edit" method="post">
                <input type="text" name="idCus" value="${listCus.idCus}${idCus}" style="display: none">
                <div class="form-group">
                    <label for="inputNameCus">Full name</label>
                    <input value="${listCus.nameCus}${name}" type="text" class="form-control" id="inputNameCus"
                           name="inputNameCus">
                    <label class="text-danger" for="inputNameCus">${msgName}</label>
                </div>
                <div class="form-group">
                    <label for="inputDateOfBirth">Date of birth</label>
                    <input value="${listCus.dateOfBirth}${date}" type="date" class="form-control" id="inputDateOfBirth"
                           name="dateOfBirthCus">
                    <label class="text-danger" for="inputDateOfBirth">${msgDate}</label>
                </div>
                <div class="form-group">
                    <label for="inputID">ID card</label>
                    <input value="${listCus.idCard}${idCard}" type="text" class="form-control" id="inputID"
                           name="idCard">
                    <label class="text-danger" for="inputID">${msgID}</label>
                </div>
                <div class="form-group">
                    <label for="inputPhone">Phone</label>
                    <input value="${listCus.phone}${phone}" type="text" class="form-control" id="inputPhone"
                           name="phoneCus">
                    <label class="text-danger" for="inputPhone">${msgPhone}</label>
                </div>
                <div class="form-group">
                    <label for="exampleInputEmailEmployee">Email address</label>
                    <input value="${listCus.email}${email}" type="email" class="form-control"
                           id="exampleInputEmailEmployee" aria-describedby="emailHelpEmployee" name="emailCus">
                    <small id="emailHelpEmployee" class="form-text text-muted">We'll never share your email with anyone
                        else.</small>
                    <label class="text-danger" for="exampleInputEmailEmployee">${msgEmail}</label>
                </div>
                <div class="form-group">
                    <label for="inputAddressCus">Address</label>
                    <input value="${listCus.address}${address}" type="text" class="form-control" id="inputAddressCus"
                           name="inputAddressCus">
                    <label class="text-danger" for="inputAddressCus">${msgAddress}</label>
                </div>
                <div class="form-group">
                    <label for="inputTypeOfCus">Type of customer</label>
                    <select class="form-control form-select" id="inputTypeOfCus" name="idTypeCus">
                        <option ${idTypeCus == "1" ? 'selected="selected"' : ''} ${listCus.typeOfCus == "1" ? 'selected="selected"' : ''}
                                value="1">Diamond
                        </option>
                        <option ${idTypeCus == "2" ? 'selected="selected"' : ''} ${listCus.typeOfCus == "2" ? 'selected="selected"' : ''}
                                value="2">Platinum
                        </option>
                        <option ${idTypeCus == "3" ? 'selected="selected"' : ''} ${listCus.typeOfCus == "3" ? 'selected="selected"' : ''}
                                value="3">Gold
                        </option>
                        <option ${idTypeCus == "4" ? 'selected="selected"' : ''} ${listCus.typeOfCus == "4" ? 'selected="selected"' : ''}
                                value="4">Silver
                        </option>
                        <option ${idTypeCus == "5" ? 'selected="selected"' : ''} ${listCus.typeOfCus == "5" ? 'selected="selected"' : ''}
                                value="5">Member
                        </option>
                    </select>
                </div>
